...
func (m *mockKernelSyscall) UnixOpen(path string, mode int, perm uint32) (fd int, err error) {
	m.unixOpenCallFlag = true
	m.unixOpenName = path
	m.unixOpenFlag = mode
	m.unixOpenPerm = perm
	return m.unixOpenFd, m.unixOpenErr
}
